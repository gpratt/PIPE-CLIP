...
class CLIP:
    ...
    def printCrosslinkingSites(self):
        ...
        if self.type == 0:
            ...
            output_sub.close()
            output_ins.close()
            return [output + "_insertion_crosslinking.pipeclip.txt", output + "_deletion_crosslinking.pipeclip.txt",
    ...
    def updateCurrentGroup(self, read, mlen, mis):
        ...
        if mlen >= self.previousQul[0] and read.mapq >= self.previousQul[1] and mis <= self.previousQul[
            2]:  # consider to append or replace only when read has no worse quality
            if mlen > self.previousQul[0] or read.mapq > self.previousQul[1] or mis < self.previousQul[
                2]:  # read has better quality,replace
    ...
    def updateCluster(self, read):
        ...
        newRead = Alignment.ClusterBed(self.originalBAM.getrname(read.tid), read.pos, read.pos + len(read.seq),
                                       clusterName, 1, strandDic[str(read.is_reverse)])
                ...
                for mutation in self.sigMutations[cluster.chr]:
                    ...
                    if cluster.overlap(mutation):
                        ...
                        if self.crosslinking.has_key(cross_key):
                            ...
                            self.crosslinking[cross_key] = Alignment.CrosslinkingBed(cluster.chr, cluster.start,
                                                                                     cluster.stop, cluster.name,
                                                                                     cluster.score, cluster.strand,
                                                                                     cluster.pvalue, cluster.qvalue,
                                                                                     mutation.start, mutation.name,
                                                                                     mutation.pvalue)
                            self.crosslinkingMutations.append(mutation)
